...
static gboolean handle_pipeline_message(GstBus *bus, GstMessage *msg, gpointer video_decoder)
{
    ...
    case GST_MESSAGE_STREAM_START:
    {
        ...
        GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(decoder->pipeline),
                                  GST_DEBUG_GRAPH_SHOW_ALL | GST_DEBUG_GRAPH_SHOW_FULL_PARAMS | GST_DEBUG_GRAPH_SHOW_STATES,
                                  filename);
        g_free(filename);
        break;
    }
...
static gchar *find_best_hw_plugin(const gchar *dec_name)
{
    ...
    {
        feature_name = !dec_name ? g_strconcat(plugins[i], "postproc", NULL) : g_strconcat(plugins[i], dec_name, "dec", NULL);
...
static gboolean create_pipeline(SpiceGstDecoder *decoder, bool try_hw_pipeline)
{
    ...
    playbin = playbin3_supported ? gst_element_factory_make("playbin3", "playbin") : NULL;
    if (playbin == NULL)
    {
